...
/**
 * E2E — Journey TermsAndConditionsCallback (Tier 2 — server required)
 * Android parity: TermsAndConditionsCallbackE2ETest
 * Server journey tree: TermsAndConditionCallbackTest
 *
 * Flow:
 *   1. Page Node                     → NameCallback:0 + PasswordCallback:0  (login)
 *   2. DataStoreDecision             → (true)
 *   3. AcceptTermsAndConditionsNode  → TermsAndConditionsCallback:0  (boolean Switch)
 *   4. SuccessNode
 */
...
const TREE = 'TermsAndConditionCallbackTest';
...
describe('Journey — TermsAndConditionsCallback', () => {
  const ensureTermsCallbackVisible = async (): Promise<void> => {
    try {
    ...
    await waitFor(element(by.id('journey-field-TermsAndConditionsCallback:0'))).toBeVisible().withTimeout(NET_TIMEOUT);
  };
  ...
  it('accept terms and submit → reaches SuccessNode (live)', async () => {
    if (!hasCallbackTreesEnabled() || !hasJourneyEnv()) { console.warn(SKIP_REASON); return; }
    await ensureTermsCallbackVisible();
    await element(by.id('journey-field-TermsAndConditionsCallback:0')).tap();
    await element(by.id('journey-submit-btn')).tap();
