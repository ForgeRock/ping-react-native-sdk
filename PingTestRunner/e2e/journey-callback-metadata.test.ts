...
/**
 * E2E — Journey MetadataCallback (Tier 2 — server required)
 * Android parity: MetadataCallbackE2ETest
 * Server journey tree: MetadataCallbackTest
 *
 * Flow:
 *   1. Page Node             → NameCallback:0 + PasswordCallback:0  (login)
 *   2. Metadata + Choice     → MetadataCallback:0 + ChoiceCallback:0
...
const TREE = 'MetadataCallbackTest';
...
describe('Journey — MetadataCallback', () => {
  const ensureMetadataAndChoiceVisible = async (): Promise<void> => {
    try {
  ...
  it('start() and login surfaces Metadata + Choice callbacks', async () => {
    if (!hasCallbackTreesEnabled() || !hasJourneyEnv()) { console.warn(SKIP_REASON); return; }
    await ensureMetadataAndChoiceVisible();
  ...
  it('select Yes and submit → reaches SuccessNode (live)', async () => {
    if (!hasCallbackTreesEnabled() || !hasJourneyEnv()) { console.warn(SKIP_REASON); return; }
    await ensureMetadataAndChoiceVisible();
    await element(by.id('journey-field-ChoiceCallback:0-option-0')).tap();
    await element(by.id('journey-submit-btn')).tap();
