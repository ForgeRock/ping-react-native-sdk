...
/**
 * E2E — Journey Name/Password callback split flow (Tier 2 — server required)
 * Android parity: NamePasswordCallbackE2ETest
...
const TREE = 'NamePasswordCallbackTest';
...
describe('Journey — NamePasswordCallback', () => {
  const ensurePasswordFieldVisible = async (): Promise<void> => {
    try {
  ...
  it('start() surfaces NameCallback', async () => {
  ...
  it('submit password → reaches SuccessNode (live)', async () => {
    if (!hasCallbackTreesEnabled() || !hasJourneyEnv()) { console.warn(SKIP_REASON); return; }
    await ensurePasswordFieldVisible();
