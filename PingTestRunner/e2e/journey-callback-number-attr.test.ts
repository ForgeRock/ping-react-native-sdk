...
/**
 * E2E — Journey NumberAttributeInputCallback (Tier 2 — server required)
 * Android parity: NumberAttributeInputCallbackE2ETest
 * Server journey tree: NumberAttributeInputCallbackTest
 *
 * Flow:
 *   1. Page Node           → NameCallback:0 + PasswordCallback:0  (login)
 *   2. DataStoreDecision   → (true)
 *   3. AttributeCollector  → NumberAttributeInputCallback:0  (custom_age)
 *   4. SuccessNode
 */
...
const TREE = 'NumberAttributeInputCallbackTest';
...
describe('Journey — NumberAttributeInputCallback', () => {
  const ensureNumberFieldVisible = async (): Promise<void> => {
    try {
      await waitFor(element(by.id('journey-field-NumberAttributeInputCallback:0')))
        .toBeVisible()
        .withTimeout(1500);
      return;
    } catch {
    ...
    await waitFor(element(by.id('journey-field-NumberAttributeInputCallback:0'))).toBeVisible().withTimeout(NET_TIMEOUT);
  };
  ...
  it('enter a number and submit → reaches SuccessNode (live)', async () => {
    if (!hasCallbackTreesEnabled() || !hasJourneyEnv()) { console.warn(SKIP_REASON); return; }
    await ensureNumberFieldVisible();
    await element(by.id('journey-field-NumberAttributeInputCallback:0')).typeText('25');
    await element(by.id('journey-submit-btn')).tap();
