...
/**
 * E2E — Journey ValidatedCreatePasswordCallback (Tier 2 — server required)
 * Android parity: ValidatePasswordCallbackE2ETest
 * Server journey tree: ValidatedPasswordCallbackTest
 *
 * Flow:
 *   1. ValidatedPasswordNode  → ValidatedCreatePasswordCallback:0  (new password to validate)
...
describe('Journey — ValidatedCreatePasswordCallback', () => {
  const ensureLoginFormVisible = async (): Promise<void> => {
    try {
      ...
      // Continue with start/validated-password flow.
    }
