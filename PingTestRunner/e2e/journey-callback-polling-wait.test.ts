...
/**
 * E2E — Journey PollingWaitCallback (Tier 2 — server required)
 * Android parity: PollingWaitCallbackE2ETest (pollingWaitCallbackExitTest)
 * Server journey tree: PollingWaitCallbackTest
 *
 * Flow:
 *   1. Page Node        → NameCallback:0 + PasswordCallback:0  (login)
 *   2. DataStoreDecision → (true)
 *   3. PollingWaitNode  → PollingWaitCallback:0 (output) + ConfirmationCallback:0 ("Exit" option)
 *   4. Tap Exit (option 0 = YES) → EXITED outcome → SuccessNode
 *
 * Timing note: the server wait is 5 s. Step 3 finds the Exit button and taps it in the same
...
const TREE = 'PollingWaitCallbackTest';
...
describe('Journey — PollingWaitCallback', () => {
  beforeAll(async () => {
  ...
  it('login → PollingWait appears → tap Exit immediately → reaches SuccessNode (live)', async () => {
    ...
    // Wait for Exit option — once visible, tap immediately to beat the 5 s server timeout
    await waitFor(element(by.id('journey-field-ConfirmationCallback:0-option-0'))).toBeVisible().withTimeout(NET_TIMEOUT);
