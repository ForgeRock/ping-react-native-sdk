...
/**
 * E2E — Journey BooleanAttributeInputCallback (Tier 2 — server required)
 * Android parity: BooleanAttributeInputCallbackE2ETest
 * Server journey tree: BooleanAttributeInputCallbackTest
 *
 * Flow:
 *   1. Page Node           → NameCallback:0 + PasswordCallback:0  (login)
 *   2. Attribute Collector → BooleanAttributeInputCallback:0       (marketing toggle)
...
const TREE = 'BooleanAttributeInputCallbackTest';
...
describe('Journey — BooleanAttributeInputCallback', () => {
  const ensureBooleanCallbackVisible = async (): Promise<void> => {
    try {
      await waitFor(element(by.id('journey-field-BooleanAttributeInputCallback:0')))
        .toBeVisible()
        .withTimeout(1500);
      return;
    } catch {
    ...
    await waitFor(element(by.id('journey-field-BooleanAttributeInputCallback:0'))).toBeVisible().withTimeout(NET_TIMEOUT);
  };
  ...
  it('toggle switch and submit → reaches SuccessNode (live)', async () => {
    if (!hasCallbackTreesEnabled() || !hasJourneyEnv()) { console.warn(SKIP_REASON); return; }
    await ensureBooleanCallbackVisible();
    await element(by.id('journey-field-BooleanAttributeInputCallback:0')).tap();
    await element(by.id('journey-submit-btn')).tap();
