...
describe('parseMfauthUri', () => {
  function loadParseMfauthUri() {
    jest.resetModules();
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    return require('@ping-identity/rn-oath').parseMfauthUri as (
      uri: string,
    ) => { oathUri?: string; pushUri?: string };
  }

  it('exports parseMfauthUri as a function', () => {
    jest.resetModules();
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const mod = require('@ping-identity/rn-oath');
    expect(typeof mod.parseMfauthUri).toBe('function');
  });

  it('extracts oathUri and pushUri from a valid mfauth:// URI', () => {
    const parseMfauthUri = loadParseMfauthUri();
    const oathEncoded = encodeURIComponent(
      'otpauth://totp/Issuer:account?secret=JBSWY3DPEHPK3PXP',
    );
    const pushEncoded = encodeURIComponent(
      'pushauth://push/Issuer:account?secret=ABC',
    );
    const uri = `mfauth://totp/Issuer:account?oathURI=${oathEncoded}&pushURI=${pushEncoded}`;
  ...
  it('returns only oathUri when pushURI is absent', () => {
    const parseMfauthUri = loadParseMfauthUri();
    const oathEncoded = encodeURIComponent(
      'otpauth://totp/Issuer:account?secret=JBSWY3DPEHPK3PXP',
    );
    const uri = `mfauth://totp/Issuer:account?oathURI=${oathEncoded}`;
    const result = parseMfauthUri(uri);
    expect(result.oathUri).toBe(
      'otpauth://totp/Issuer:account?secret=JBSWY3DPEHPK3PXP',
    );
  ...
  it('returns {} for a non-mfauth URI', () => {
    const parseMfauthUri = loadParseMfauthUri();
  ...
  it('returns {} for an invalid URI', () => {
    const parseMfauthUri = loadParseMfauthUri();
    expect(parseMfauthUri('not a uri at all')).toEqual({});
  });

  it('returns {} for an empty string', () => {
    const parseMfauthUri = loadParseMfauthUri();
    expect(parseMfauthUri('')).toEqual({});
  });
});
