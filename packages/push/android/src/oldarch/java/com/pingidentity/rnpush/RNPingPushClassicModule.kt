...
/**
 * Classic bridge entry point for the Push module (legacy architecture).
 *
 * Delegates all operations to [RNPingPushCommon].
 */
@ReactModule(name = RNPingPushClassicModule.NAME)
class RNPingPushClassicModule(
  ...
  /** Returns the registered module name used by the classic React Native bridge. */
  ...
  /**
   * Creates and registers a new [com.pingidentity.mfa.push.PushClient] from the bridge config map.
   *
   ...
   */
  @ReactMethod
  fun saveCredential(clientId: String, credential: ReadableMap, promise: Promise) {
   ...
   */
  @ReactMethod
  fun approveChallengeNotification(
    clientId: String,
   ...
   */
  @ReactMethod
  fun approveBiometricNotification(
    clientId: String,
    notificationId: String,
  ...
  companion object {
    const val NAME = "RNPingPushClassic"
  }
}
